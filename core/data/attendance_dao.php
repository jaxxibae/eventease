...
class AttendanceDAO
{
    ...
    {
        $stmt = $this->db->prepare('SELECT Attendance.Id, e.Id as EventId, e.BannerUrl as BannerUrl, e.Name as EventName, e.EventDate as EventDate, e.EventTime as EventTime, Status, Motive FROM Attendance JOIN Events e ON Attendance.EventId = e.Id WHERE UserId = :userId AND Status = :status ORDER BY Attendance.Id ASC');
        $status = 'Confirmed';
        $stmt->bindParam(':userId', $user_id);
